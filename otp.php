...
if(!empty($_POST["submit_email"])) {
	$result = mysqli_query($conn,"SELECT * FROM registration WHERE email='" . $_POST["email"] . "'");
	...
	if($count>0) {
		...
		$result = mysqli_query($conn,"INSERT INTO otp_expiry (`otp`,`create_at`) VALUES ('" . $otp . "','" . date("Y-m-d H:i:s"). "')");
		// exit();
		if($result) {
			$success=1;
		}
